<?php
/**
 * Plugin Name: Woo Nova Poshta Pro
...
function np_cheeckout_update_review($posted_data)
{
...
add_action('woocommerce_checkout_update_order_review', 'np_cheeckout_update_review');
...
function np_get_price_shipping($citycost)
{
    ...
    if (isset($_COOKIE['billing_city']))
    {
        $citycost = $_COOKIE['billing_city'];
    }
...
function pnp_adjust_shipping_rate($rates)
{
    ...
    foreach ($rates as $rate)
    {
        ...
        if (isset($_COOKIE['billing_city']))
        {
            $billing_city = $_COOKIE['billing_city'];
        }
...
class NovattnPoshta extends Base
{
    ...
    public function isNPttn()
    {
        ...
        $sessionMethods = WC()
            ->session->chosen_shipping_methods;
    ...
    public function scripts()
    {
        ...
        if ($load)
        {
            ...
            wp_register_script('nova-poshta-js', NOVA_POSHTA_TTN_SHIPPING_PLUGIN_URL . $fileName, ['jquery-ui-autocomplete'], filemtime(NOVA_POSHTA_TTN_SHIPPING_PLUGIN_DIR . $fileName));
            ...
            wp_enqueue_script('select3', NOVA_POSHTA_TTN_SHIPPING_PLUGIN_URL . '/assets/select3.min.js', array(
                'jquery'
            ) , MNP_PLUGIN_VERSION, true);
            wp_enqueue_script('nova-poshta-js', array(
                'jquery',
                'select3'
            ));
    ...
    public function styles()
    {
        ...
        if ($load)
        {
            ...
            global $wp_scripts;
            $jquery_version = isset($wp_scripts->registered['jquery-ui-core']
                ->ver) ? $wp_scripts->registered['jquery-ui-core']->ver : '1.9.2';
    ...
    public function adminStyles()
    {
        ...
        wp_register_style('nova-poshta-style', NOVA_POSHTA_TTN_SHIPPING_PLUGIN_URL . $fileName, ['jquery-ui-style'], filemtime(NOVA_POSHTA_TTN_SHIPPING_PLUGIN_DIR . $fileName));
    ...
    public function adminScripts()
    {
        ...
        wp_register_script('nova-poshta-admin-js', NOVA_POSHTA_TTN_SHIPPING_PLUGIN_URL . $fileName, ['jquery-ui-autocomplete'], filemtime(NOVA_POSHTA_TTN_SHIPPING_PLUGIN_DIR . $fileName));
    ...
    {
        wp_localize_script($handle, 'NovaPoshtaHelper', ['ajaxUrl' => admin_url('admin-ajax.php', 'relative') , 'textforcostcalc' => __('Розрахунок вартості доставки', NOVA_POSHTA_TTN_DOMAIN) , 'textforcostcalcafter' => __('(оплата за доставку відбувається на відділенні нової пошти)', NOVA_POSHTA_TTN_DOMAIN) , 'textforcostcalcen' => 'Delivery cost calculation', 'chooseAnOptionText' => __('Choose an option', NOVA_POSHTA_TTN_DOMAIN) , 'getRegionsByNameSuggestionAction' => AjaxRoute::GET_REGIONS_BY_NAME_SUGGESTION, 'getCitiesByNameSuggestionAction' => AjaxRoute::GET_CITIES_BY_NAME_SUGGESTION, 'getWarehousesBySuggestionAction' => AjaxRoute::GET_WAREHOUSES_BY_NAME_SUGGESTION, 'getCitiesAction' => AjaxRoute::GET_CITIES_ROUTE, 'getWarehousesAction' => AjaxRoute::GET_WAREHOUSES_ROUTE, 'markPluginsAsRated' => AjaxRoute::MARK_PLUGIN_AS_RATED, ]);
    }
