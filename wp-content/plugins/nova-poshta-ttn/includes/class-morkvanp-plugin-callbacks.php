 ...
 */
 class MNP_Plugin_Callbacks {
  ...
  public function morkvanpCheckoutExample1(){
    ...
    //echo '<pre>';
    $value = esc_attr( get_option( 'morkvanp_checkout_auto' ) );
    //print_r( $value );
    //echo '</pre>';

    echo '<fieldset>
  <legend>Виберіть методи оплати:</legend>';

  foreach ($enabled_gateways as $key) {
    $checked = $value == $key[0] ? "checked" : "unchecked";
    echo '
    <div>
      <input type="checkbox" id="'.$key[0].'" name="morkvanp_checkout_auto" '.$checked.' value="'.$key[0].'">
      <label for="'.$key[0].'">'.$key[1].'</label>
    </div>';
...
echo '</fieldset>';
    ...
    echo '<p>Буде враховано останній вибраний варіант</p>';
...
public function morkvanpttn2(){
$activate = get_option( 'ttn2_example' );

$checked = $activate;
$current = 1;
$echo = false;
echo '<input '. $activate .' type="checkbox" class="regular-text" name="ttn2_example" value="1" ' . checked($checked, $current, $echo) . ' />За замовчуванням ттн лише створюються для замовлень, в яких обрано метод доставки нова пошта, проте якщо для оформлення замовлення використовується інший плагін, позначте ццей пункт </p>';
}
...
$current = 1;
$echo = false;
echo '<input '. $activate .' type="checkbox" class="regular-text" name="plus_calc" value="1" ' . checked($checked, $current, $echo) . ' />Сума доставки буде сплачена при оформленні. Можуть виникнути колізії з наложеним платежем</p>';
}
